...
class EdgesSuite extends FunSuite {
  ...
  test("Edge.doIntersect - collinear cases") {
    ...
    assert(
      edgeA_00_22.doIntersect(edgeB_11_33),
      "Collinear overlapping edges ((0,0)-(2,2)) and ((1,1)-(3,3)) should intersect"
    )
    ...
    assert(
      !edgeC_00_11.doIntersect(edgeD_22_33),
      "Collinear non-overlapping edges ((0,0)-(1,1)) and ((2,2)-(3,3)) should not intersect"
    )
    ...
    assert(
      edgeE_00_33.doIntersect(edgeF_11_22),
      "Collinear edge ((0,0)-(3,3)) containing ((1,1)-(2,2)) should intersect"
    )
    ...
    assert(
      edgeG_00_11.doIntersect(edgeH_11_22),
      "Collinear edges ((0,0)-(1,1)) and ((1,1)-(2,2)) touching at endpoint should intersect"
    )
    ...
    assert(
      edgeI_00_11.doIntersect(edgeJ_22_11),
      "Collinear edges ((0,0)-(1,1)) and ((2,2)-(1,1)) sharing endpoint (one reversed) should intersect"
    )
    assert(
      edgeJ_22_11.doIntersect(edgeI_00_11),
      "Intersection should be commutative for collinear shared endpoint (one reversed)"
    )
    ...
    assert(
      !edgeK_m11_00.doIntersect(edgeL_11_22),
      "Collinear non-overlapping edges ((-1,-1)-(0,0)) and ((1,1)-(2,2)) should not intersect"
    )
  }
  ...
  test("Edge.doIntersect - T-intersections and degenerate edges") {
    ...
    assert(
      edgeT2_10_11.doIntersect(edgeT1_01_21),
      "Intersection should be commutative for T-intersection (endpoint on edge)"
    )
    ...
    assert(
      edgeT3_01_31.doIntersect(edgeT4_10_12),
      "Standard T-junction ((0,1)-(3,1)) and ((1,0)-(1,2)) should intersect"
    )
    ...
    // Normal N1: (0,0)-(2,2)
    assert(
      degenP11.doIntersect(normalEdge_00_22),
      "Degenerate edge (1,1) on normal edge ((0,0)-(2,2)) should intersect"
    )
    ...
    // Normal N1: (0,0)-(2,2)
    assert(
      !degenP33.doIntersect(normalEdge_00_22),
      "Degenerate edge (3,3) not on normal edge ((0,0)-(2,2)) should not intersect"
    )
    ...
    // Degenerate D2: (3,3)-(3,3)
    assert(
      !degenP11.doIntersect(degenP33),
      "Two degenerate edges at different points (1,1) and (3,3) should not intersect"
    )
  }
}
