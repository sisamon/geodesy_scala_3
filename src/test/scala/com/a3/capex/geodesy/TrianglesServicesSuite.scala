...
class TrianglesServicesSuite extends FunSuite {
  ...
  test("Delaunay with 3 collinear points should throw RuntimeException (or return empty list depending on Tinfour behavior)") {
    val p1 = pt(0, 0)
    val p2 = pt(1, 1) 
    val p3 = pt(2, 2)
    val points = List(p1, p2, p3)
  ...
  test("Delaunay with 5 points") {
    ...
    assert(triangles.forall(t => List(t.p1, t.p2, t.p3).contains(p5)), "Center point p5 should be part of all triangles")
  }
}
