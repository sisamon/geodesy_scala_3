...
import com.a3.capex.geodesy.ShapesCore.{Box, Point}
import com.a3.capex.geodesy.Coordinates.{Latitude, Longitude}
import squants.space.AngleConversions.* // For .degrees
import squants.space.AreaConversions.*  // For .squareMeters etc.
...
class ShapesCoreSuite extends FunSuite {
  ...
  test("Point.isSorted should correctly compare points") {
    ...
    val p3_lat10_lon15 =
      Point(Latitude.unsafeMake(10.degrees), Longitude.unsafeMake(15.degrees)) // lat1 == lat3, lon1 > lon3
    ...
    assert(
      !p4_lat10_lon20.isSorted(p1_lat10_lon20),
      "An identical point should not be sorted before the original point"
    )
  }
  ...
  test("Point.orientation should determine orientation of three points") {
    ...
    assertEquals(
      Point.orientation(p00, p10, Point(Latitude.unsafeMake(2.degrees), Longitude.unsafeMake(0.degrees))),
      0,
      "Horizontal points should be collinear"
    )
    assertEquals(
      Point.orientation(p00, p01, Point(Latitude.unsafeMake(0.degrees), Longitude.unsafeMake(2.degrees))),
      0,
      "Vertical points should be collinear"
    )
  ...
  test("Box.area should calculate the correct area") {
    ...
    val pNE_zero_width =
      Point(Latitude.unsafeMake(10.degrees), Longitude.unsafeMake(10.degrees)) // north = 10, east = 10
    ...
    val pNE_zero_height =
      Point(Latitude.unsafeMake(10.degrees), Longitude.unsafeMake(20.degrees)) // north = 10, east = 20
    val pSW_zero_height =
      Point(Latitude.unsafeMake(10.degrees), Longitude.unsafeMake(10.degrees)) // south = 10, west = 10
  ...
  test("Box.contains should correctly check for containment of another Box") {
    ...
    val boxStd = Box(
      Point(Latitude.unsafeMake(10.degrees), Longitude.unsafeMake(20.degrees)),
      Point(Latitude.unsafeMake(0.degrees), Longitude.unsafeMake(10.degrees))
    ) // N:10, S:0, E:20, W:10, Width:10

    // Antimeridian crossing box
    val boxAM = Box(
      Point(Latitude.unsafeMake(10.degrees), Longitude.unsafeMake(-170.degrees)),
      Point(Latitude.unsafeMake(0.degrees), Longitude.unsafeMake(170.degrees))
    ) // N:10, S:0, E:-170(190), W:170, Width:20

    // Test cases for boxStd
    val identicalToStd = Box(
      Point(Latitude.unsafeMake(10.degrees), Longitude.unsafeMake(20.degrees)),
      Point(Latitude.unsafeMake(0.degrees), Longitude.unsafeMake(10.degrees))
    )
    assert(boxStd.contains(identicalToStd), "boxStd should contain identical box")

    val smallerInStd = Box(
      Point(Latitude.unsafeMake(8.degrees), Longitude.unsafeMake(18.degrees)),
      Point(Latitude.unsafeMake(2.degrees), Longitude.unsafeMake(12.degrees))
    ) // N:8,S:2,E:18,W:12, W:6
    assert(boxStd.contains(smallerInStd), "boxStd should contain smaller box inside")

    val largerThanStd = Box(
      Point(Latitude.unsafeMake(12.degrees), Longitude.unsafeMake(22.degrees)),
      Point(Latitude.unsafeMake(-5.degrees), Longitude.unsafeMake(5.degrees))
    ) // N:15,S:-5,E:30,W:5, W:25
    assert(!boxStd.contains(largerThanStd), "boxStd should NOT contain larger box")

    val partiallyOverlappingStd = Box(
      Point(Latitude.unsafeMake(5.degrees), Longitude.unsafeMake(15.degrees)),
      Point(Latitude.unsafeMake(-5.degrees), Longitude.unsafeMake(5.degrees))
    ) // N:5,S:-5,E:15,W:5, W:10
    assert(
      !boxStd.contains(partiallyOverlappingStd),
      "boxStd should NOT contain partially overlapping box (south outside)"
    )

    val disjointFromStd = Box(
      Point(Latitude.unsafeMake(30.degrees), Longitude.unsafeMake(40.degrees)),
      Point(Latitude.unsafeMake(20.degrees), Longitude.unsafeMake(30.degrees))
    )
    assert(!boxStd.contains(disjointFromStd), "boxStd should NOT contain disjoint box")

    val touchingInternallyStd = Box(
      Point(Latitude.unsafeMake(10.degrees), Longitude.unsafeMake(15.degrees)),
      Point(Latitude.unsafeMake(0.degrees), Longitude.unsafeMake(12.degrees))
    ) // N:10,S:0,E:15,W:12, W:3
    assert(boxStd.contains(touchingInternallyStd), "boxStd should contain internally touching box")

    val touchingExternallyStd = Box(
      Point(Latitude.unsafeMake(10.degrees), Longitude.unsafeMake(30.degrees)),
      Point(Latitude.unsafeMake(0.degrees), Longitude.unsafeMake(20.degrees))
    ) // N:10,S:0,E:30,W:20, W:10
    ...
    val smallerInAM = Box(
      Point(Latitude.unsafeMake(8.degrees), Longitude.unsafeMake(-175.degrees)),
      Point(Latitude.unsafeMake(2.degrees), Longitude.unsafeMake(175.degrees))
    ) // N:8,S:2,E:-175,W:175, W:10
    assert(boxAM.contains(smallerInAM), "boxAM should contain smaller box inside (AM)")

    val largerThanAM = Box(
      Point(Latitude.unsafeMake(12.degrees), Longitude.unsafeMake(-160.degrees)),
      Point(Latitude.unsafeMake(-2.degrees), Longitude.unsafeMake(160.degrees))
    ) // N:15,S:-5,E:-160,W:160, W:40
    ...
    val notCrossingAMButWithinLongitudesOfBoxAM = Box(
      Point(Latitude.unsafeMake(5.degrees), Longitude.unsafeMake(10.degrees)),
      Point(Latitude.unsafeMake(2.degrees), Longitude.unsafeMake(175.degrees))
    )
    ...
    assert(
      !boxAM.contains(notCrossingAMButWithinLongitudesOfBoxAM),
      "boxAM should NOT contain box that is technically within its lon range but doesn't cross AM itself if boxAM does"
    )

    val widerThanAM = Box(
      Point(Latitude.unsafeMake(5.degrees), Longitude.unsafeMake(178.degrees)),
      Point(Latitude.unsafeMake(2.degrees), Longitude.unsafeMake(-160.degrees))
    ) // N:5,S:2,E:160,W:-160 (spans 40deg, but other way around)
    ...
    val amBoxInsideStd = Box(
      Point(Latitude.unsafeMake(5.degrees), Longitude.unsafeMake(-170.degrees)),
      Point(Latitude.unsafeMake(2.degrees), Longitude.unsafeMake(172.degrees))
    ) // N:5,S:2,E:-170,W:170, W:20
    ...
    val amBoxContainsStd = Box(
      Point(Latitude.unsafeMake(15.degrees), Longitude.unsafeMake(9.degrees)),
      Point(Latitude.unsafeMake(-5.degrees), Longitude.unsafeMake(21.degrees))
    ) // N:15,S:-5,E:9,W:21 (crosses AM, e.g. 21 to 360 then 0 to 9)
    ...
    val amBoxContainsStdNarrow = Box(
      Point(Latitude.unsafeMake(15.degrees), Longitude.unsafeMake(25.degrees)),
      Point(Latitude.unsafeMake(-5.degrees), Longitude.unsafeMake(5.degrees))
    ) // N:15,S:-5,E:25,W:5 (normal box)
