...
import squants.space.{Kilometers, Meters, SquareKilometers, SquareMeters, Area, Length}
...
class TrianglesSuite extends FunSuite {
  ...
  test("Triangle.apply(Point) should create a degenerate triangle") {
    ...
    assert(Math.abs(tri.area.toSquareMeters - 0.0) <= epsilonArea.toSquareMeters, "Degenerate triangle area should be zero")
  }
  ...
  test("area of three distinct collinear points should be zero") {
    ...
    assert(Math.abs(Triangle(p1,p2,p3).area.toSquareMeters - 0.0) <= epsilonArea.toSquareMeters, "Area of 3 distinct collinear points should be ~0")
  }
  ...
  test("area of a spherical octant (1/8 of sphere surface)") {
    ...
    assert(Math.abs(tri.area.toSquareMeters - expectedAreaSqMeters) <= (expectedAreaSqMeters * 0.001), "Area of spherical octant") // 0.1% tolerance
  }
