...
import Coordinates.{zero, Latitude, Longitude, R}
...
object ShapesCore:
  /** We will create an abstract class to act as root for any geometry we need. It has some basic geometry built on. It
    * also provides some quick functions to help with things like intersections.
    */
  ...
  trait CurvedShapes extends Serializable:
    ...
    /** Distance between this geometry and the given point using harversine.
      *
      * The distance is measured in terms of the closest point in the geometry. For points this is obvious (there is
      * only one point to use). For boxes, this means that points contained within the box (or on the perimeter) have a
      * distance of zero.
      */
    def distance(pt: Point): Length =
      val dLon: Double =
        if pt.longitude < east then (pt.longitude - east).toRadians
        else if pt.longitude < west then 0f
        else (west - pt.longitude).toRadians
      val dLat: Double =
        if pt.latitude < south then (pt.latitude - south).toRadians
        else if pt.latitude < north then 0f
        else (north - pt.latitude).toRadians
      val a = pow(sin(dLat / 2.0), 2) + pow(sin(dLon / 2.0), 2) * cos(north.unwrap.toRadians) * cos(
        pt.latitude.unwrap.toRadians
      )
      val c = 2 * asin(sqrt(a))
      R * c

    /** Squared distance should be used when we are interested in ordering, we trade the actual value for spped, while
      * retaining the ranking.
      */
    def distanceSquared(pt: Point): Double =
      val dx: Angle =
        if pt.longitude < east then east - pt.longitude
        else if pt.longitude < west then zero
        else pt.longitude - west
      val dy: Angle =
        if pt.latitude < south then south - pt.latitude
        else if pt.latitude < north then zero
    ...
    private def isLongitudeInsideEffectiveSpan(
        lon: Longitude,
        effWest: Double,
        effEast: Double,
        effCrossesAM: Boolean
    ): Boolean = {
      val lonDeg = lon.unwrap.toDegrees
      if effCrossesAM then {
    ...
    /** Returns whether this geometry contains the other.
      *
      * Containment includes the border, so points "on the edge" count as contained. This logic now considers the
      * "smaller span" interpretation for longitude ranges.
      */
...
//      println(s"  [lonCheck] geom.east_raw (${geom.east.unwrap.toDegrees}) inside THIS_eff (W:$thisEffW, E:$thisEffE, AM:$thisCrossesAM): $geomEastInsideThis")
      ...
      val longitudinalSpanContained =
        if !thisCrossesAM && geomCrossesAM then
...
//      println(s"[Box.contains] =======================================================================")
    ...
    /** Return the given geometry's area outside this geometry. This is equivalent to the area that would be added by
      * expand(). Calculation is obviously wrong, need check formulas for this.
      */
    ...
    /** Returns whether this geometry wraps the other.
      *
      * This is the same thing as containment, but it excludes the border. Points can never wrap anything, and boxes can
      * only wrap geometries with less area than they have.
      */
  ...
  case class Point(latitude: Latitude, longitude: Longitude) extends CurvedShapes:
    ...
    def *(other: Double): Point.PointKey =
      Point.PointKey((other * this.latitude.unwrap.toDegrees).toLong, (other * this.longitude.unwrap.toDegrees).toLong)

    override def distance(pt: Point): Length =
      val dLon = pt.longitude - longitude
      val dLat = pt.latitude - latitude
      val a = pow((dLat / 2).sin, 2) + pow((dLon / 2).sin, 2) * cos(latitude.unwrap.toRadians) * cos(
        pt.latitude.unwrap.toRadians
      )
      val c = 2 * asin(sqrt(a))
      R * c
    ...
    def sorted(p: Point): (Point, Point) =
      if this.isSorted(p) then (this, p) else (p, this)

    // Sorted by latitude, used to ensure membership of edges
    def isSorted(p: Point): Boolean =
      if (latitude < p.latitude) || (latitude == p.latitude && longitude < p.longitude) then true else false
    ...
    def azimuthTo(another: Point): Angle =
      ...
      if this.keyCoordinates == another.keyCoordinates then 0.degrees
      else if this.longitude.unwrap =~ another.longitude.unwrap then
        // if (printme) println("mismalongitud")
        if this.latitude < another.latitude then 0.degrees else 180.degrees
      else if this.latitude.unwrap =~ another.latitude.unwrap then
        // if (printme) println("mismalongitud")
        if this.longitude < another.longitude then 90.degrees else -90.degrees
      else
        ...
        (if t < 0.0 then t + 360.0 else t).degrees
      end if
    end azimuthTo
  end Point
  ...
  object Point:
    ...
    def orientation(p: Point, q: Point, r: Point): Int =
      ...
      if value == 0 then 0     // Collinear
      else if value > 0 then 1 // Clockwise
  ...
  case class Box(northEast: Point, southWest: Point) extends CurvedShapes:
    ...
    override def barycenter: Point =
      (northEast + southWest) / 2.0 // Point((north + south) / 2.0, ((east + west) / 2.0))
    override def isDegenerate: Boolean = northEast == southWest

  // Is this actually needed anymore
  object Box:
    /** This is an "inside-out" box that we use as a good starting value. The nice thing about this, unlike
      * Box(0,0,0,0), is that when merging with another box we don't include an artificial "empty" point.
      */
    val empty: Box =
