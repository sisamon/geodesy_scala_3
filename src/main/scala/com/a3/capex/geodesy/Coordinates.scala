...
/** @author
  *   Luis Sisamon We are going to work on decimal degrees, we can always add UTM or other coordinates systems, but it
  *   is simpler to define the logic using this approach.
  */
...
object Coordinates:
  ...
  /** Let's take advantage of Scalas's type system to avoid swapping lat and lon coordinates accidentally. This is also
    * important because latitude goes from -90 to +90, while longitude goes from -180 to 180. Besides that the behavior
    * of latitude is "normal", that is a lower latitude is always lower, south of, we will never have any structure
    * going from 85 to -85. For longitude is more ambiguous, a rectangle from 179 to -179 may happen and we can not use
    * the lower criteria to order west to east. Of course this may be an issue when transposing, but I see no use to
  ...
  /** Represents a longitude value in degrees, constrained to valid ranges. Valid ranges are either -180.0 to +180.0
    * (with negative for West, positive for East) or 0.0 to 360.0 (with 0 at Greenwich, increasing Eastward).
    *
    * Note: When working near the antimeridian (±180°), special care must be taken with operations like min/max and
    * comparisons, as the values wrap around.
    */
  ...
  object Longitude extends Newtype[Angle]:
    ...
    override inline def validate(value: Angle): TypeValidation =
      ...
      then true
      else "Longitude must be between -180.0 and +180.0 or between 0.0 and +360.0"

  /** Represents a latitude value in degrees, constrained to -90.0 to +90.0. Negative values represent the Southern
    * hemisphere, positive values the Northern. The poles are represented by -90.0 (South) and +90.0 (North).
    */
  ...
  object Latitude extends Newtype[Angle]:
    /** Validates that a latitude value is within -90.0 to +90.0 degrees.
      ...
      */
    override inline def validate(value: Angle): TypeValidation =
      if value.toDegrees >= -90.0 && value.toDegrees <= 90.0 then true
      else "Latitude must be between -90.0 and +90.0"

  /** Extension methods for Latitude values */
  extension (lat: Latitude)
    /** Calculates the average between two latitudes. This is useful for finding midpoints or centers.
      *
      * @example
      *   val lat1 = Latitude.unsafeMake(10.degrees) val lat2 = Latitude.unsafeMake(30.degrees) val avg =
      *   lat1.average(lat2) // ~20 degrees
      */
    ...
    /** Adds two latitude values together. Note: Uses unsafeMake because the result is normalized to be within valid
      * latitude range.
      * @deprecated
      *   Use `average` for calculating midpoints or add Angle for displacement
      */
    ...
    /** Adds an angle to a latitude. Note: Uses unsafeMake because the result is normalized to be within valid latitude
      * range.
      */
    ...
    /** Subtracts an angle from a latitude. Note: Uses unsafeMake because the result is normalized to be within valid
      * latitude range.
      */
    def -(other: Angle): Latitude =
      // Safe to use unsafeMake here because normalize ensures the result is within valid range
    ...
    /** Calculates the angular difference between two latitudes. Returns an Angle representing the difference.
      */
    ...
    /** Returns the more northern of two latitudes.
      * @param other
      *   The other latitude to compare with
      * @return
      *   The more northern latitude
      */
    @targetName("latmax")
    def max(other: Latitude): Latitude =
      if lat.unwrap.toDegrees >= other.unwrap.toDegrees then lat else other

    /** Returns the more southern of two latitudes.
      * @param other
      *   The other latitude to compare with
      * @return
      *   The more southern latitude
      */
    @targetName("latmin")
    ...
    /** Compares two latitudes for ordering.
      ...
      *   The other latitude to compare with
      * @return
      *   0 if equal, 1 if this latitude is north of other, -1 if south
      */
  ...
  extension (lon: Longitude)
    /** Calculates the average between two longitudes, handling the antimeridian correctly. This is useful for finding
      * midpoints between two points on the globe.
      *
      * @example
      *   val lon1 = Longitude.unsafeMake(170.degrees) val lon2 = Longitude.unsafeMake(-170.degrees) val avg =
      *   lon1.average(lon2) // 180 degrees (not 0 degrees)
      *
      * @param other
      *   The other longitude to average with
      * @return
      *   The average longitude, correctly handling the antimeridian
      */
    ...
    def average(other: Longitude): Longitude =
      ...
      val avg =
        ...
        else ((lon1 + lon2) / 2).degrees
      Longitude.unsafeMake(avg)

    /** Adds two longitude values together, with proper normalization. Note: Uses unsafeMake because the result is
    ...
    /** Adds an angle to a longitude, with proper normalization. Note: Uses unsafeMake because the result is normalized
      * to be within -180° to +180°.
      */
    ...
    /** Calculates the angular difference between two longitudes. Returns an Angle representing the smallest difference,
    ...
    /** Subtracts an angle from a longitude, with proper normalization. Note: Uses unsafeMake because the result is
      * normalized to be within -180° to +180°.
      */
    ...
    /** Compares two longitudes for ordering, following a westward criteria. This handles the circular nature of
      * longitudes, being careful around the antimeridian.
      *
      * @param other
      *   The other longitude to compare with
      * @return
      *   0 if equal, 1 if this is west of other (and less than 180° away), -1 if this is east of other or more than
      *   180° west
      */
    ...
    /** Returns the more easterly of two longitudes, accounting for the circular nature and the shortest path between
      * them.
      *
      * @param other
      *   The other longitude to compare with
      * @return
      *   The more easterly longitude
      */
    @targetName("lonmax")
    def max(other: Longitude): Longitude = {
      val diff = (other.unwrap.toDegrees - lon.unwrap.toDegrees + 540) % 360 - 180
      if diff > 0 && diff <= 180 then other else lon
    }

    /** Returns the more westerly of two longitudes, accounting for the circular nature and the shortest path between
      * them.
      *
      * @param other
      *   The other longitude to compare with
      * @return
      *   The more westerly longitude
      */
    @targetName("lonmin")
    ...
    /** Normalizes a longitude angle to the range [-180°, +180°]. This handles wrapping around the antimeridian
      * correctly.
      *
      * @param value
    ...
    /** Ensures the longitude is in the standard range of -180° to +180°. This is useful after operations that might
      * produce values outside this range.
      *
      * @return
      *   A new Longitude normalized to [-180°, +180°]
      * @note
      *   Uses unsafeMake because we're just adjusting the range, not changing the actual value
      */
    def denormalize(): Longitude =
      // Safe to use unsafeMake here because we're just adjusting the range
      Longitude.unsafeMake(lon.unwrap + (if lon.unwrap > Degrees(180.0) then Degrees(-360.0) else Degrees(0.0)))
  end extension

  /** Opaque type representing a longitude value optimized for use as a map key. This is a lossy representation that
    * provides fast hashing and comparison. The value represents degrees * 1e6 (microdegrees) as a Long.
    */
  ...
  /** Opaque type representing a latitude value optimized for use as a map key. This is a lossy representation that
    * provides fast hashing and comparison. The value represents degrees * 1e6 (microdegrees) as a Long.
    */
...
object CoordinatesImplicits:
  import com.a3.capex.geodesy.Coordinates.{Latitude, LatitudeKey, Longitude, LongitudeKey} // Longitude360,
