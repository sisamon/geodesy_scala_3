...
import com.a3.capex.geodesy.ShapesCore.{Box, CurvedShapes, Point}
import com.a3.capex.geodesy.CoordinatesImplicits.{given_Conversion_Latitude_Angle, given_Conversion_Longitude_Angle}

object Edges:

  case class Edge(origin: Point, end: Point) extends CurvedShapes:
    override def barycenter: Point = Point(
      Latitude.unsafeMake((origin.latitude + end.latitude) / 2),
      Longitude.unsafeMake((origin.longitude + end.longitude) / 2)
    )
    ...
    def sorted(): Edge =
      if origin.isSorted(end) then Edge(origin, end) else Edge(end, origin)
